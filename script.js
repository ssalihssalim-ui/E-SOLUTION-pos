...
async function initApp() {
    console.log('🚀 E-SOLUTION - Point de Vente Started');
...
function buildMenu() {
    ...
    if (window.currentUserData && window.currentUserData.userData.role === 'admin') {
        items = [
            ...
            { p: 'products', i: 'fa-box', l: 'Produits' },
...
function navigateTo(page) {
    ...
    var icons = { dashboard: 'fa-chart-line', pos: 'fa-cash-register', commandes: 'fa-shopping-basket', categories: 'fa-layer-group', products: 'fa-box', clients: 'fa-users', fournisseurs: 'fa-truck', ventes: 'fa-shopping-cart', credits: 'fa-credit-card', depenses: 'fa-money-bill-wave', statistiques: 'fa-chart-bar', options: 'fa-cog' };
    ...
    content.innerHTML = '<div style="text-align:center;padding:20px;"><i class="fas fa-spinner fa-spin" style="font-size:1.5rem;color:#14B8A6;"></i></div>';
...
window.loadCreditsPage = window.loadCreditsPage || async function(c) {
    ...
    c.innerHTML = '<div class="content-card">' +
        ...
        '</div>' +
        '<input type="text" id="creditsVoiceDisplay" placeholder="🎤 Audio..." style="padding:8px 12px; border:2px solid #14B8A6; border-radius:8px; width:180px; background:#f0fdf4; color:#0D9488; font-weight:600;" readonly>' +
        ...
        '</div></div>' +
        '<div id="creditPaymentZone" style="display:none; background:#f0fdf4; border:2px solid #14B8A6; border-radius:12px; padding:12px 16px; margin-bottom:15px;">' +
        '<div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px;">' +
        '<div><strong>💳 Paiement en cours</strong><br><span id="creditPaymentInfo" style="font-size:0.85rem; color:#0D9488;">Aucun crédit sélectionné</span></div>' +
        '<div style="display:flex; align-items:center; gap:10px;">' +
        '<label style="font-weight:600; font-size:0.9rem;">Montant :</label>' +
        '<input type="number" id="creditPaymentAmountInput" placeholder="0.00" step="0.01" style="width:120px; padding:6px 10px; border:2px solid #14B8A6; border-radius:8px; font-size:0.9rem;" onfocus="this.select();">' +
...
function updateSidebarUserInfo() { var el = document.getElementById('sidebarUserInfo'); if (el && window.currentUserData) { el.innerHTML = '<i class="fas fa-user-circle"></i> ' + window.currentUserData.userData.prenom + ' ' + window.currentUserData.userData.nom + ' <small style="color:#14B8A6;">(' + window.currentUserData.userData.role + ')</small>'; } }
