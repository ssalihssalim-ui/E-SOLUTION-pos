...
function loadStatistiquesPage(c) {
    ...
    <div class="content-card" style="margin-bottom:15px;">
        <div class="card-header">
            <h3><i class="fas fa-chart-bar"></i> Tableau de bord - Statistiques E-SOLUTION</h3>
...
async function loadStatistiques() {
    ...
    try {
        ...
        statsHTML += buildStatCard('Chiffre d\'affaires', totalVentes.toFixed(2) + ' MAD', 'fa-money-bill-wave', '#dcfce7', '#14B8A6');
        statsHTML += buildStatCard('Ventes', nbVentes.toString(), 'fa-shopping-cart', '#e0e7ff', '#4f46e5');
        statsHTML += buildStatCard('Profit brut', totalProfit.toFixed(2) + ' MAD', 'fa-chart-line', '#fef3c7', '#14B8A6');
        ...
        statsHTML += buildStatCard('Clients', nbClients.toString(), 'fa-users', '#dcfce7', '#14B8A6');
        statsHTML += buildStatCard('Produits', nbProduits.toString(), 'fa-box', '#f0fdf4', '#14B8A6');
        statsHTML += buildStatCard('Taux conversion', tauxConversion.toFixed(1) + '%', 'fa-chart-pie', '#fef3c7', '#14B8A6');
        ...
        if (fideliteSettings.active) statsHTML += buildStatCard('Points fidélité', pointsTotal.toString(), 'fa-star', '#fef3c7', '#14B8A6');
        statsHTML += '</div>';
        ...
        else statsHTML += '<tr><td colspan="2">Aucune vente</td></tr>';
        ...
        setTimeout(() => {
            try {
                ...
                if (ctx1) statsCharts.sales = new Chart(ctx1, { type: 'line', data: { labels: Object.keys(dailySales), datasets: [{ label: 'CA (MAD)', data: Object.values(dailySales), borderColor: '#14B8A6', backgroundColor: 'rgba(20,184,166,0.1)', fill: true, tension: 0.3, pointRadius: 3, pointBackgroundColor: '#14B8A6' }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { callback: v => v + ' MAD' } } } } });
                var ctx2 = document.getElementById('paymentChart')?.getContext('2d');
                if (ctx2) statsCharts.payment = new Chart(ctx2, { type: 'doughnut', data: { labels: Object.keys(paymentMethods), datasets: [{ data: Object.values(paymentMethods), backgroundColor: ['#14B8A6', '#4f46e5', '#16a34a', '#ef4444', '#d97706'] }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } } } });
                var ctx3 = document.getElementById('categoryChart')?.getContext('2d');
                if (ctx3) statsCharts.category = new Chart(ctx3, { type: 'bar', data: { labels: topCategories.map(c => c[0]), datasets: [{ label: 'CA (MAD)', data: topCategories.map(c => c[1]), backgroundColor: '#14B8A6', borderRadius: 5 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { callback: v => v + ' MAD' } } } } });
...
console.log('🚀 E-SOLUTION - Statistics JS prêt');
